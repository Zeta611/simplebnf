...
\AtBeginDocument{%
  \usepackage[
    only, fatsemi, fatslash, fatbslash, talloblong, rightarrowtriangle
  ]{stmaryrd}%
}
...
\title{%
  ...
  \footnote{This file describes v1.2.0.}}
\author{Jay Lee\footnote{E-mail: %
  \href{mailto:jay.lee@berkeley.edu}{\texttt{jay.lee@berkeley.edu}}}}
\date{2026/07/01}
...
\def\colonincolon{:in:}
\def\doubleslash{//}
\def\doublebslash{\char`\\\char`\\}
\def\metabnfcomment{\char`\\bnfcomment}
\begin{tcolorbox}[sharp corners]
  \begin{center}
    \begin{bnf}(
      ...
      ! \texttt{\metabnfcomment\char`{$c$\char`}} // full-line comment
      ! \texttt{\metabnfcomment\char`{$c$\char`}\,$P$} // commented production
    ;;;
      P // Production :::= $L \rightarrowtriangle R$
    ;;;
      L // LHS :::=
      ! $v$ // metavariable
      ! $v\!\fatslash\,c$ // commented metavariable
    ;;;
      R // RHS :::=
      ! $A$ // alternative
    ...
    ;;;
      A // Alternative :::=
      ! $f$ // syntactic form
      ! $f\fatbslash A$ // line-broken syntactic form
      ! $f\!\fatslash\,c$ // commented syntactic form
    ;;;
    ...
    ;;;
      ...
      ! | // new-line delimiter
      ! \doubleslash // single-line delimiter
      ! $\cdots$ // user-defined
    ;;;
      \fatbslash // Line-break delim :::=
      ! \doublebslash // default symbol
      ! $\cdots$ // user-defined
    ;;;
...
Some examples of customizable symbols include the delimiters for productions and alternatives, the relation symbol between the left-hand side and the right-hand side, the comment symbol, and more.
...
The configuration can also be set globally using \cmd{SetBNFConfig}, similar to \cmd{SetBNFLayout}:
\begin{listing}
% Some where above...
\SetBNFConfig{
  prod-delim = {--},
...
For consecutive alternatives delimited by the single-line alternative delimiter, only the last of them can be commented.

Note that the single-line alternative delimiter must not contain the new-line alternative delimiter as a substring.

\subsection{Line-break delimiter}
The line-break delimiter \verb/line-break-delim/ defaults to \verb/\\/.
It forces a manual line break inside a single alternative of the right-hand side, aligning the continuation under the RHS column without repeating the relation or the or-symbol.
...
\subsection{Comment delimiter}
The default comment delimiter \verb/comment/ is \verb/:/.
This will separate the syntactic form and the comment within the alternative.
...
\begin{example}[righthand width=.51\linewidth]
\begin{center}
\begin{bnf}(
  prod-delim = {--},
  or-sym = {},
...
\subsection{Full-line comments}\label{subsec:comment}
To insert a full-line comment row inside a grammar, use \cmd{bnfcomment}:
\begin{example}
\begin{center}
\begin{bnf}
  \bnfcomment{Types}
...
Another subtlety to consider while customizing delimiters, is that certain delimiters should not contain the others as a substring.
